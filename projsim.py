...
# Projection Simulation Function
def projection_simulation():
    st.title('Fantasy Football Projection Simulator')

    draft_results_df = st.file_uploader("Upload your draft results CSV file", type=["csv"])

    if draft_results_df is not None:
        draft_results_df = pd.read_csv(draft_results_df)
        ...
        # Parameters for the projection simulation
        num_simulations_payout = st.number_input("Number of simulations for payout", min_value=1, value=10)
        
        if st.button("Run Payout Simulations"):
            final_results = run_parallel_simulations(num_simulations_payout, draft_results_df, projection_lookup)

            # Display the payout results
            st.write("Payout Simulation Results")
            st.dataframe(final_results)

            # Download link for the payout results
            payout_csv = final_results.to_csv(index=False).encode('utf-8')
            st.download_button(
